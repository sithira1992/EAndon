...
(function (angular) {
    ...
    angular.module('andonControllers')
        ...
        .controller("orderApprovalCtrl", ['$scope', '$http','$routeParams',  '$filter', '$location','$rootScope',
...
// or server returns response with an error status.
                ...
                $scope.Delete= function (detail) {    //submit button


                    $scope.msgs = [];
                    $http.post('db/RequestForm.php?action=delete_request',{'id':detail.id,
                        'quantity':detail.Quantity,'status':'0','position':$rootScope.globals.currentUser.position}).success(function(data, status, headers, config) {
                        if (data.msg != '')
                        {


                            msgs="**Order Denied Successfully**"
                            $scope.get_Order_details();
                            $scope.messege='Order Denied Successfully ';
                            $scope.notify('success');
                            $scope.msgs.push(msgs);
                        }
                        else
